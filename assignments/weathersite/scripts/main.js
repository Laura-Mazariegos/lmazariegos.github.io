...
weatherObject.open('GET','https://api.wunderground.com/api/06755379febc9626/conditions/q/MN/Franklin.json', true );
weatherObject.send();
...
var greenvilleWeather = new XMLHttpRequest();
	greenvilleWeather.open('GET','https://api.wunderground.com/api/06755379febc9626/conditions/q/TX/Greenville.json', true );
	greenvilleWeather.send();
	greenvilleWeather.onload = function() {
		var weatherInfo = JSON.parse(greenvilleWeather.responseText);
		console.log(weatherInfo);
		document.getElementById('place').innerHTML = weatherInfo.current_observation.display_location.full;
		document.getElementById('currentTemp').innerHTML = weatherInfo.current_observation.temp_f;
		document.getElementById('feelsLike').innerHTML = weatherInfo.current_observation.feelslike_string;
...
var springfieldWeather = new XMLHttpRequest();		springfieldWeather.open('GET','https://api.wunderground.com/api/06755379febc9626/conditions/q/OR/Springfield.json', true );
		springfieldWeather.send();
		springfieldWeather.onload = function() {
			var weatherInfo = JSON.parse(springfieldWeather.responseText);
			console.log(weatherInfo);
			document.getElementById('place').innerHTML = weatherInfo.current_observation.display_location.full;
			document.getElementById('currentTemp').innerHTML = weatherInfo.current_observation.temp_f;
			document.getElementById('feelsLike').innerHTML = weatherInfo.current_observation.feelslike_string;
